Shader "stillalive-studios/Triplanar/Triplanar Bumped Specular Top-Bottom"
{
	Properties
	{
		...
		_MainColor_Bottom ("Bottom Color (RGB)", color) = (1,1,1,1)
		_ScaleUV_Bottom("Bottom: UV Scale", Float) = 1
		_TexDiff_Bottom ("Bottom: Diffuse (RGB) + Height (A)", 2D) = "white" {}
		_TexBump_Bottom ("Bottom: Normal Map", 2D) = "bump" {}
		_TexSpec_Bottom ("Bottom: Specularity Strength (G) + Glossiness (A)", 2D) = "black" {}
...
//		#pragma debug
		...
		fixed4 _MainColor_Bottom;
		sampler2D _TexDiff_Bottom;
		sampler2D _TexBump_Bottom;
		sampler2D _TexSpec_Bottom;
		float _ScaleUV_Bottom;
		...
		void surf(Input IN, inout SurfaceOutput o) 
		{
			...
			float2 uvTopBottom = float2(IN.worldPos.x,-IN.worldPos.z);
			
			//sample bottom texture
			float3 bottomDiff;
			float3 bottomNrml;
			float2 bottomSpec;
			SampleTexWithSpec(bottomDiff, bottomNrml, bottomSpec,
				uvTopBottom * _ScaleUV_Bottom, _TexDiff_Bottom, _TexBump_Bottom, _TexSpec_Bottom);
			bottomDiff *= _MainColor_Bottom.rgb;
			
			//sample top textures
			SampleTexWithSpec(colorMatrix[1], nrmlMatrix[1], specMatrix[1],
				uvTopBottom * _ScaleUV_Top, _TexDiff_Top, _TexBump_Top, _TexSpec_Top);
			colorMatrix[1] *= _MainColor_Top.rgb;
			
			//now mix them together
			float bottomTopLerp = step(IN.worldNrml.y, 0);
			colorMatrix[1] = lerp(colorMatrix[1], bottomDiff, bottomTopLerp);
			nrmlMatrix[1] = lerp(nrmlMatrix[1], bottomNrml, bottomTopLerp);
			specMatrix[1] = lerp(specMatrix[1], bottomSpec, bottomTopLerp);
